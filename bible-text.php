...
Copyright (c) 2009-2017 Mark Barnes
...
function mbbt_add_net_text ($reference, $heading) {
    $net_url = "http://labs.bible.org/api/?type=xml&passage=".str_replace(" ", "+", $reference);
    if (!class_exists('SimpleXMLElement') || $heading === FALSE || strtolower($heading) == 'span')
        $xml = mbbt_get_xml(wp_remote_retrieve_body(wp_remote_get($net_url.'&formatting=plain')));
    else
        $xml = mbbt_get_xml(wp_remote_retrieve_body(wp_remote_get($net_url.'&formatting=para')));
    $output = '';
    $old_chapter = 0;
    $items = array();
    $items = $xml->item;
